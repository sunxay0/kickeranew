...
const TabButton: React.FC<{ active: boolean; onClick: () => void; children: React.ReactNode }> = ({ active, onClick, children }) => (
  <button
    onClick={onClick}
    className={`flex-1 p-2 text-xs font-semibold transition-colors flex items-center justify-center gap-1 ${
      active ? 'bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'
    }`}
  >
    {children}
  </button>
...
export const FieldDetailPanel: React.FC<FieldDetailPanelProps> = ({ field, onClose, onUpdateField, currentUser, onOpenChat, onUpdateUser, onCheckIn, onCheckOut, onViewProfile }) => {
  ...
  return (
    <div className={`absolute top-0 right-0 h-full w-full md:w-96 bg-gray-50/95 dark:bg-gray-900/90 backdrop-blur-sm shadow-2xl z-20 transform transition-transform duration-300 ease-in-out border-l border-gray-200 dark:border-gray-700 ${field ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          {activeTab === 'players' && (
            <ul className="space-y-3 p-4">
              {field.players.map(player => (
                <li key={player.id}>
                  <button 
                    onClick={() => player.id !== currentUser.id && onViewProfile(player)}
