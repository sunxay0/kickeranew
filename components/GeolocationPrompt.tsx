...
export const GeolocationPrompt: React.FC<GeolocationPromptProps> = ({ status, onAllow, onSkip }) => {
  ...
  if (status === 'loading' || status === 'initial') {
    content = (
      <>
        <LoadingSpinner />
        <p className="mt-4 text-lg">{t('gettingLocation')}</p>
      </>
    );
    ...
    content = (
      <>
        <LocationMarkerIcon />
        <h1 className="text-2xl font-bold mt-4">{t('locationAccessDenied')}</h1>
        <p className="text-gray-500 dark:text-gray-400 mt-2 max-w-sm text-center">
          {t('locationPermissionsPrompt')}
        </p>
    ...
    content = (
      <>
        <LocationMarkerIcon />
        <h1 className="text-2xl font-bold mt-4">{t('findFieldsNearYou')}</h1>
        <p className="text-gray-500 dark:text-gray-400 mt-2 max-w-sm text-center">
