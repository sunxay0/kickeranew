...
export const PlayerCard: React.FC<PlayerCardProps> = ({ 
  ...
  const getRarityText = (rarity: string) => {
    switch (rarity) {
      case 'bronze':
        return t('bronze_card');
      case 'silver':
        return t('silver_card');
      case 'gold':
        return t('gold_card');
      case 'special':
        return t('special_card');
      default:
      ...
      <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-white/80 text-sm">{getRarityText(playerCard.rarity)}</p>
        ...
        {playerCard.specialAbilities.length > 0 && (
          <div className="mt-4">
            <h4 className="text-white font-semibold text-sm mb-2">{t('specialAbilities')}:</h4>
        ...
        {playerCard.isSpecial && playerCard.validUntil && (
          <div className="mt-3 p-2 bg-yellow-400/20 rounded-lg">
            <p className="text-yellow-200 text-xs text-center">
              {t('validUntil')}: {new Date(playerCard.validUntil).toLocaleDateString()}
            </p>
          </div>
        )}
      </div>
