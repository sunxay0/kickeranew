...
const ManageTournamentView: React.FC<{
    ...
    const handleScoreChange = (matchId: string, teamIndex: 0 | 1, score: string) => {
        const newMatches = matches.map(m => {
            if (m.id === matchId) {
                const newScores: [number | null, number | null] = [...m.scores];
                newScores[teamIndex] = score === '' ? null : parseInt(score, 10);
                // FIX: Explicitly define the type for the new status to prevent TypeScript
                // from widening it to `string`, which would violate the `Match` type.
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto flex-grow p-4">
                ...
                <FormRow label={t('tournamentFormat')}><FormSelect value={tournamentFormat} onChange={e => setTournamentFormat(e.target.value as TournamentFormat)}>{Object.values(TournamentFormat).map(f => <option key={f} value={f}>{t(tournamentFormatMap[f])}</option>)}</FormSelect></FormRow>
                <FormRow label={t('teamFormat')}><FormSelect value={teamFormat} onChange={e => setTeamFormat(e.target.value as TeamFormat)}>{Object.values(TeamFormat).map(f => <option key={f} value={f}>{t(teamFormatMap[f])}</option>)}</FormSelect></FormRow>
